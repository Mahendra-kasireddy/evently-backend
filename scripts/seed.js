...
async function seedPackages() {
    ...
    for (const p of packages) {
        await coll.updateOne({ title: p.title }, // natural key
        { $set: { ...p, updatedAt: new Date() }, $setOnInsert: { createdAt: new Date() } }, { upsert: true });
    }
...
async function seedOrganizers() {
    const coll = mongoose_1.default.connection.collection('organizer_profiles');
    for (const o of organizers) {
        await coll.updateOne({ name: o.name }, // natural key
        { $set: { ...o, updatedAt: new Date() }, $setOnInsert: { createdAt: new Date() } }, { upsert: true });
    }
    const count = await coll.countDocuments();
...
async function seedDemoUser() {
    ...
    await users.updateOne({ _id: user._id }, { $set: { name: 'Priya Reddy', city: 'Hyderabad, Telangana', updatedAt: new Date() } });
...
async function seedDemoBooking() {
    ...
    const bookings = mongoose_1.default.connection.collection('bookings');
    await bookings.updateOne({ customer: user._id, ref: 'EVT-2026-8841' }, {
        $set: {
            customer: user._id,
            ref: 'EVT-2026-8841',
            title: 'Your Wedding · 28 Dec 2026',
            description: 'Sharma Events is managing every vendor. Review the plan, approve your invitation, and track progress — all in one workspace.',
...
const customerHomeContent = {
    ...
    hero: {
        ...
        headingTail: ' next?',
        subtitle: 'Tell us the occasion — verified organizers send tailored quotes within a day. You compare, you choose, they handle everything.',
...
async function seedPlanConfig() {
    ...
    for (const { collection, key, docs } of sets) {
        ...
        for (const doc of docs) {
            await coll.updateOne({ [key]: doc[key] }, // natural key
...
async function seedContent() {
    const coll = mongoose_1.default.connection.collection('site_content');
    await coll.updateOne({ key: 'customer-home' }, {
        $set: { key: 'customer-home', data: customerHomeContent, updatedAt: new Date() },
        $setOnInsert: { createdAt: new Date() },
    }, { upsert: true });
    await coll.updateOne({ key: 'customer-plan' }, {
...
async function seedDemoNotifications() {
    const phone = process.env.SEED_DEMO_PHONE;
    if (!phone)
        return; // silent — booking step already explains the flag
    const users = mongoose_1.default.connection.collection('users');
    const user = await users.findOne({ phone });
    if (!user)
        return;
    const notifications = mongoose_1.default.connection.collection('notifications');
    const demo = [
        {
            type: 'quote',
    ...
    for (const n of demo) {
        await notifications.updateOne({ user: user._id, title: n.title }, {
